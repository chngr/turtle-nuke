...
public class Combat{
  ...
  private int allyAdjacentCost = 10;
  private int enemyAdjacentCost = 3;
  ...
  public void setAggression(int aggressionFactor){
    enemyAdjacentCost = (int)((r.getEnergon()/40.0) * aggressionFactor);
  }
  ...
  private void updateMap() throws GameActionException{
    ...
    for(Robot robot: nearbyRobots){
      ...
      if(Math.abs(dx) < 3 && Math.abs(dy) < 3){
        ...
        localMap[2 + dx][2 + dy] = ri.team == r.getTeam() ? allyTile : enemyTile;
      }
    }
  }
